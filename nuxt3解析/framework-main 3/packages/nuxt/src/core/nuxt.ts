...
async function initNuxt(nuxt: Nuxt) {
  ...
  nuxt.hook("prepare:types", (opts) => {
    ...
    opts.references.push({
      path: resolve(nuxt.options.buildDir, "types/plugins.d.ts"),
    });
    // Add vue shim
    if (nuxt.options.typescript.shim) {
      opts.references.push({
        path: resolve(nuxt.options.buildDir, "types/vue-shim.d.ts"),
      });
    }
    // Add module augmentations directly to NuxtConfig
    opts.references.push({
      path: resolve(nuxt.options.buildDir, "types/schema.d.ts"),
    });
    opts.references.push({
      path: resolve(nuxt.options.buildDir, "types/app.config.d.ts"),
    });
  });
  ...
  // Add unctx transform
  addVitePlugin(
    UnctxTransformPlugin(nuxt).vite({ sourcemap: nuxt.options.sourcemap })
  );
  addWebpackPlugin(
    UnctxTransformPlugin(nuxt).webpack({ sourcemap: nuxt.options.sourcemap })
  );
  ...
  if (!nuxt.options.dev) {
    ...
    addVitePlugin(
      TreeShakePlugin.vite({
        sourcemap: nuxt.options.sourcemap,
        treeShake: removeFromServer,
      }),
      { client: false }
    );
    addVitePlugin(
      TreeShakePlugin.vite({
        sourcemap: nuxt.options.sourcemap,
        treeShake: removeFromClient,
      }),
      { server: false }
    );
    addWebpackPlugin(
      TreeShakePlugin.webpack({
        sourcemap: nuxt.options.sourcemap,
        treeShake: removeFromServer,
      }),
      { client: false }
...
export async function loadNuxt(opts: LoadNuxtOptions): Promise<Nuxt> {
  ...
  options.alias["@vue/composition-api"] = resolve(
    options.appDir,
    "compat/capi"
  );
