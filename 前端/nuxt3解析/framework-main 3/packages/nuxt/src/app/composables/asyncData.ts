...
>(
  ...
  let [key, handler, options = {}] = args as [
    string,
    (ctx?: NuxtApp) => Promise<DataT>,
  ...
  const useInitialCache = () =>
    (nuxt.isHydrating || options.initialCache) &&
    nuxt.payload.data[key] !== undefined;

  const asyncData = {
    data: ref(
      useInitialCache() ? nuxt.payload.data[key] : options.default?.() ?? null
    ),
  ...
  asyncData.refresh = (opts = {}) => {
    ...
    nuxt._asyncDataPromises[key] = new Promise<DataT>((resolve, reject) => {
      try {
