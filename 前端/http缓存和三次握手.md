...
1. 传输方式：1.1 是文本传输，解析慢；2.0 是二进制帧传输，解析更快、更稳定。
2. 并发处理：1.1 单连接有队头阻塞，只能开多连接；2.0 支持多路复用，单连接可并行处理多个请求，无阻塞。
3. 头部传输：1.1 每次请求都带完整 header，冗余大；2.0 用 HPACK 算法压缩头部，节省带宽。
4. 服务器推送：2.0 支持服务器主动推送资源，1.1 只能被动响应。
5. 请求优先级：2.0 可设置请求优先级，核心资源优先加载，1.1 无此功能。
6. 总结：HTTP2.0 没有改变 HTTP1.1 的核心语义（如请求方法、状态码），主要是优化传输效率，解决 1.1 的性能瓶颈。
