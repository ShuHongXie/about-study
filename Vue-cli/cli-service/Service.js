...
module.exports = class Service {
  constructor(context, { plugins, pkg, inlineOptions, useBuiltIn } = {}) {
    console.log(context, { plugins, pkg, inlineOptions, useBuiltIn });
    ...
    console.log(this.plugins, this.pkgContext, this.commands);
  }
  ...
  init(mode = process.env.VUE_CLI_MODE) {
    // 加载mode 当为build时 mode = production
    console.log("-----mode");
    ...
    // load mode .env
    // 加载当前模式下的全局变量 如 .env.production
    if (mode) {
      this.loadEnv(mode);
    }
    // load base .env 加载基础的.env
    this.loadEnv();

    // load user config
    // 加载用户的配置项
    const userOptions = this.loadUserOptions();
    // 混合用户配置 形成最终配置 以用户配置为优先
    this.projectOptions = defaultsDeep(userOptions, defaults());

    debug("vue:project-config")(this.projectOptions);

    // apply plugins.
    // require所有组件 也就是require所有插件
  ...
  resolvePlugins(inlinePlugins, useBuiltIn) {
    ...
    console.log(inlinePlugins, useBuiltIn);
    const idToPlugin = (id) => ({
      // 把所有./开头的 转换成 built-in:
      // 这里主要把下面builtInPlugins的执行命令路径转换成 built-in:path
      id: id.replace(/^.\//, "built-in:"),
      apply: require(id),
    });

    // isPlugin = /^(@vue\/|vue-|@[\w-]+(\.)?[\w-]+\/vue-)cli-plugin-/.test(依赖名称)
    ...
    if (inlinePlugins) {
      ...
      // 引用所有vue开头的的插件，返回的对象id值为插件名，apply为require(插件名)
     ...
     *  [
        {
          ...
          apply: [Function (anonymous)] { defaultModes: [Object] }
        },
        {
        ...
        { id: 'built-in:commands/help', apply: [Function (anonymous)] },
        { id: 'built-in:config/base', apply: [Function (anonymous)] },
        { id: 'built-in:config/css', apply: [Function (anonymous)] },
        { id: 'built-in:config/prod', apply: [Function (anonymous)] },
        { id: 'built-in:config/app', apply: [Function (anonymous)] },
        { id: '@vue/cli-plugin-babel', apply: [Function (anonymous)] },
        { id: '@vue/cli-plugin-eslint', apply: [Function (anonymous)] },
        { id: '@vue/cli-plugin-router', apply: [Function (anonymous)] },
        { id: '@vue/cli-plugin-vuex', apply: [Function (anonymous)] }
      ] D:\workspace\test-temp {}
     */
  ...
  loadUserOptions() {
    ...
    if (fileConfigPath) {
      if (esm && fileConfigPath === "./vue.config.js") {
        throw new Error(
          `Please rename ${chalk.bold("vue.config.js")} to ${chalk.bold(
            "vue.config.cjs"
          )} when ECMAScript modules is enabled`
        );
      }
